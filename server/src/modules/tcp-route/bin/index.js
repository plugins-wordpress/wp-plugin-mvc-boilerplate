...
class CLI extends require("../base") {
  ...
  commands() {
    switch (this.command(2)) {
      case "make":
        const { make, makeWith } = new TCPRoute({ command: this.command(2) });
        if (this.command(3)) {
          if (this.command(4)) {
            // const tcpController = new TCPController();
            makeWith(this.command(3), this.command(4));
            // make(this.command(3), this.command(4));
            // tcpController.make(this.command(4).split('--controller=')[1])
          } else {
            make(this.command(3));
          }
        } else {
