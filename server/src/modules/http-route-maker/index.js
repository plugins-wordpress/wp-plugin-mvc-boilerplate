...
.then(files => {
   ...
   writeable.write(`\n`)
   writeable.write(`module.exports = (router = require('express').Router()) => {
    `)
    writeable.write(`\n    `)
   for(let file of files){
    // if(!file.includes('index.js')){
    ...
    if(file.split('/').pop().split('.js').join('') !== 'index'){
      // writeable.write(`${file.split('/').pop().split('.js').join('')}(app);        \n    `)
      writeable.write(`router.use(${file.split('/').pop().split('.js').join('')})();        \n    `)
    }
   }
   writeable.write(`return router \n`)
   writeable.write(`\n`)
   writeable.write(`}`)
      ...
      //  }
       ...
       for(let file of files){
        ...
        if(file.split('/').pop().split('.js').join('') !== 'index'){
          ...
          writeable.write(`router.use(require(".${file.split('.js').join('').split('routes')[1]}")());        \n    `)
        }
       }
...
`
    ...
    for (let method in methods) {
      if (method === "index") {
        ...
        module += `    router.get("/${options.router.endpoint}", ${method});\n`;
      }
      if (method === "store") {
        // console.log(`app.post("/${options.router.endpoint}", ${method});`)
        module += `    router.post("/${options.router.endpoint}", ${method});\n`;
      }
      if (method === "show") {
        // console.log(`app.get("/${options.router.endpoint}/:id", ${method});`)
        module += `    router.get("/${options.router.endpoint}/:id", ${method});\n`;
      }
      if (method === "update") {
        // console.log(`app.post("/${options.router.endpoint}/:id", ${method});`)
        module += `    router.post("/${options.router.endpoint}/:id", ${method});\n`;
      }
      if (method === "edit") {
        // console.log(`app.put("/${options.router.endpoint}/:id", ${method});`)
        module += `    router.put("/${options.router.endpoint}/:id", ${method});\n`;
      }
      if (method === "destroy") {
        // console.log(`app.delete("/${options.router.endpoint}/:id", ${method});`)
        module += `    router.delete("/${options.router.endpoint}/:id", ${method});\n`;
      }
    }
