...
|--------------------------------------------------------------------------------
|
| Team extends the base model (Model) class and thus has everything
...
const TeamFactory  = require('../factory/models/team')
...
class Team extends Model {
    ...
    |
    */
    constructor(dbOptions = { collection: 'teams', url: 'mongodb://localhost:27017/nodecraftsman', db: 'nodecraftsman' }, ...options) {
        ...
        */
        ...
        if (!this['hasOwnProperty']['collection']) this.collection = 'teams';
    ...
    */
    ...
    async userTeams(user, count = Infinity, fn = () => {}){
        try{
            const teams = await this.find({user_id: user._id.toString()}, {limit: count})
            fn(null, teams)
            this.emit('userTeams', teams)
            return teams
        }catch(error){
            fn(error, null)
            this.emit('userTeams-error', error)
            return error
        }
     ...
     populate(count = 2, owner, user = new User()) {
        ...
        if(owner){
            for(let i = 0; i < count; i++){
                this.create(TeamFactory(owner))
            }
        }else{
            user.findOne().then(async auth => {
                for(let i = 0; i < count; i++){
                    this.create(TeamFactory(auth))
                }
            }).catch(console.error)
        }

    }
    async addMember(team, member, fn = () => {}){
        try{
            const result = await this.updateById(team._id.toString(), {members: team.members.push(member._id.toString())})
            fn(null, result)
    ...
    async addMembers(team, memberList  = [], fn = () => {}){
        try{
            const result = await this.updateById(team._id.toString(), {members: memberList.map(member => member._id.toString())})
            fn(null, result)
    ...
    async removeMember(team, member, fn = () => {}){
        
        try{
            const newMemberList = team.members.filter(currentMember  => currentMember._id.toString() !== member._id.toString())
            const result = await this.updateById(team._id.toString(), {members: newMemberList})
            fn(null, result)
            this.emit('addMember', result)
            return result
        }catch(error){
    ...
    async removeMembers(team, memberList  = [], fn = () => {}){
