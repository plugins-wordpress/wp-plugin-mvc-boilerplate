...
document.addEventListener('DOMContentLoaded', function () {
  (function () {
    ...
    const selectedUserContact = (user = {}) => {
      $(`#selected-contact-status`).addClass(`avatar-${user.status}`)
      $(`#selected-contact-name`).text(`${user.firstname} ${user.lastname}`)
      $(`#selected-contact-jobTitle`).text(user.jobTitle)
      $(`#selected-contact-avatar`).attr({ src: user.avatar })
    }
    const selectedContactSidebar = (user = {}) => {

      $(`#selected-contact-sidebar-status`).addClass(`avatar-${user.status}`)
      $(`#selected-contact-sidebar-name`).text(`${user.firstname} ${user.lastname}`)
      $(`#selected-contact-sidebar-jobTitle`).text(user.jobTitle)
      $(`#selected-contact-sidebar-avatar`).attr({ src: user.avatar })
      $(`#selected-contact-sidebar-about`).text(user.bio)
      $(`#selected-contact-sidebar-email`).text(user.email)
      $(`#selected-contact-sidebar-phone`).text(user.phone)
      $(`#selected-contact-sidebar-status`).addClass(`avatar avatar-${user.status}`)
    }

    const selectedStatus = (id, value) => {

      socket.emit('user-selected-status', { id, value })
      socket.on('user-selected-status-result', user => { })
    }
    const selectedContact = (user = windown.user, id) => {
      window.localStorage.setItem('contact', JSON.stringify(id));

      socket.emit('user-selected-contact', { user, id })
      socket.on('user-selected-contact-result', contact => {
        if (!window.contact) window.contact = contact
        window.localStorage.setItem('contact', JSON.stringify(contact));
        selectedUserContact(contact)
        selectedContactSidebar(contact)
      })
      ...
      socket.on('message', chats => {
        ...
        if (chats && chats.length > 0) {
          for (let chat of chats) {
            ...
            renderMsg.innerHTML = '<p class="mb-0 text-break">' + chat.data.message + '</p>';
    ...
    // const selectedChatUser  = (id) => {
    //   socket.emit('user-selected-contact', id)
    //   socket.once('user-selected-contact-result', user => {
    //     selectedUserContact(user)
    //     selectedContactSidebar(user)
    //   })
    // }
    ...
    chatUserStatus.forEach(el => {
      el.addEventListener('click', e => {
        ...
        selectedStatus($(e.currentTarget).data('contact'), value)
    ...
    chatContactListItems.forEach(chatContactListItem => {
      ...
      const chatContactSelectionHandler = e => {
        // const contactId = $(e.target).data('contact');
        e.stopPropagation();


        if ($(e.target).data('contact')) selectedContact($(e.target).data('contact'))
      ...
      // chatContactListItem.removeEventListener('click',chatContactSelectionHandler);
      chatContactListItem.addEventListener('click', e => {
        // const contactId = $(e.target).data('contact');
        e.stopPropagation();
        if ($(e.target).data('contact')) {
          selectedContact(user, $(e.target).data('contact'))
        }
    ...
    formSendMessage.addEventListener('submit', e => {
      ...
      if (messageInput.value) {
        ...
        socket.emit('send-message', { chatSender: user, chatReceiver: contact, message: messageInput.value })
        let renderMsg = document.createElement('div');
        renderMsg.className = 'chat-message-text mt-2';
        renderMsg.innerHTML = '<p class="mb-0 text-break">' + messageInput.value + '</p>';
        document.querySelector('li:last-child .chat-message-wrapper').appendChild(renderMsg);
