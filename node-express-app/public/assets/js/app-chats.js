...
document.addEventListener('DOMContentLoaded', function () {
  (function () {
    ...
    const selectSidebarContact = contact => {
      $(`#user-sidebar-contact-status`).removeClass()
      $(`#user-sidebar-contact-status`).addClass(`avatar avatar-xl avatar-${contact.status}`)
      $(`#user-sidebar-contact-about`).text(contact.bio)
      $(`#user-sidebar-contact-avatar`).attr({
        src: contact.avatar
      })
      $(`#user-sidebar-contact-name`).text(`${contact.firstname} ${contact.lastname}`)
      $(`#user-sidebar-contact-email`).text(contact.email)
      $(`#user-sidebar-contact-phone`).text(contact.phone)
      $(`#user-sidebar-contact-job-title`).text(contact.jobTitle)
    }

    const selectedUserContact = contact => {
      $(`#user-contact-name`).text(`${contact.firstname} ${contact.lastname}`)
      $(`#user-contact-job-title`).text(contact.jobTitle)
      $(`#user-contact-avatar`).attr({
        src: contact.avatar
      })
      $(`#user-contact-status`).removeClass()
      $(`#user-contact-status`).addClass(`flex-shrink-0 avatar avatar-${contact.status}`)
    }

    window.selectedChatUser = id => {
      const contact = window.users.find(contact => contact._id === id)
      if(contact){
        window.localStorage.setItem('selected-chat-user', JSON.stringify(contact))
        selectedUserContact(contact)
        selectSidebarContact(contact)
      }
      
    }
    window.selectedContact = id => {
